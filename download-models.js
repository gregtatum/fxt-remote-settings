...
async function downloadModels() {
  for (const [languagePair, files] of Object.entries(modelRegistry)) {
    const fromLang = languagePair[0] + languagePair[1];
    const toLang = languagePair[2] + languagePair[3];
    for (const [fileType, file] of Object.entries(files)) {
      const url = `${modelRegistryRootURL}/${languagePair}/${file.name}`;
      const dirname = fileURLToPath(new URL('.', import.meta.url));
      const localPath = path.join(dirname, "models", file.name);


      console.log();
      console.log(url);
      console.log({ name: file.name, version: 1, fromLang, toLang, fileType });
